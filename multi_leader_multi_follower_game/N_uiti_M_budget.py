...
def figure(y):
    ...
    # -------------两种算法能够支持的用户比例----------------------------------
    ax.set_ylabel(r'Utility of provider $U_m$', font1)
    # ax.set_title('The bandwidth demand')
    ax.set_xlabel(r'Total budgets of requestors $\sum_{m=1}^{M} \beta_m$', font1)
    marker = [".", "^", "x", "P"]
    ls = ["--", "-.", ":", "-"]
    l_m =  np.linspace(0, 2160, 10)
    # plt.savefig('fix.eps', dpi=300)  # 指定分辨率保存
    lns = ax.plot(l_m, y[:, 0],  marker = "v", ls = "-", label= r"Provider 1 with ${b_n}^{tol} = 20$")
    for n in range(1, 5):
        lns2 = ax.plot(l_m, y[:, n], marker = marker[n-1], ls = ls[n-1], label= "Provider " + str(n+1) + " with ${b_n}^{tol} = $" + str((n+1) * 20))
        lns = lns + lns2
    labs = [l.get_label() for l in lns]
    ax.set_ylim(0, 260, 50)
    x_ticks = np.arange(0, 2400, 240)
    plt.xticks(x_ticks)
...
def figure2(y):
    ...
    ax.tick_params(labelsize=12)
    # -------------两种算法能够支持的用户比例----------------------------------
    ax.set_ylabel(r'Price of provider $\alpha_n$', font1)
    # ax.set_title('The bandwidth demand')
    ax.set_xlabel(r'Total budgets of requestors $\sum_{m=1}^{M} \beta_m$', font1)
    marker = [".", "^", "x", "P"]
    ls = ["--", "-.", ":", "-"]
    ...
    x_ticks = np.arange(0, 2400, 240)
    plt.xticks(x_ticks)
    ax.legend(lns, labs, loc = 4, prop=font2, framealpha=0.8)
    ax.grid()
    ax.margins(0)
    #plt.savefig('Utility_provider.eps', dpi=300)
    plt.show()
...
if __name__ == '__main__':
    ...
    time_budget = 10
    U_N_iter = np.zeros([time_budget, 5])
    a_N_iter = np.zeros([time_budget, 5])
    for l_m_0 in range(time_budget):
        l_m = np.linspace(4 * l_m_0, 4 * l_m_0, m)
        ...
        for ite in range(iter):
            ...
            a_n = (a_n_old + a_n) / 2
        U_N_iter[l_m_0, :] = (a_n - c_n) * np.sum(b_m_n, axis = 0)
        a_N_iter[l_m_0, :] = a_n
    figure(U_N_iter)
    figure2(a_N_iter)
    print("结束")