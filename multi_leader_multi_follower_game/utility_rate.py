...
def figure(l_m, y):
    ...
    lns = ax.plot(l_m, y[0, :],  marker = "v", ls = "-", label= r"Requestor with $\eta_m$ = 0.4")
    for n in range(1, 5):
        lns2 = ax.plot(l_m, y[n, :], marker = marker[n-1], ls = ls[n-1], label= "Requestor with $\eta_m$ = " + str((n + 4) / 10))
        lns = lns + lns2
...
if __name__ == '__main__':
    ...
    for g_m_0 in range(5):
        # g_m 为follower资源的利用率
        g_m = 0.1 * np.random.randint(4 + g_m_0, 5 + g_m_0, size=m)
        ...
        for ite in range(iter):
            ...
            a_n = (a_n_old + a_n) / 2
        U_M_iter[g_m_0, :] = w_m * np.sum(np.log(1 + np.multiply(g_m_n, b_m_n)), axis=1) - np.sum(np.multiply(b_m_n, a_m_n), axis = 1)
    figure(l_m, U_M_iter)
    print("结束")
